...
{ // anytime error, print descriptive msg;
...
void disableRawMode()
{ // sets terminal to its original attributes (canonical
  // mode);
...
void enableRawMode()
{ // rawmode (texteditor mode, unlike canonical mode);
...
int editorReadKey()
{ // wait for one keypress, return it;
  int nread;
  char c;
...
{ // find width, height of terminal
  struct winsize ws;
  if (ioctl(STDOUT_FILENO, TIOCGWINSZ, &ws) == -1 || ws.ws_col == 0)
  {
    if (write(STDOUT_FILENO, "\x1b[999C\x1b[999B", 12) !=
        12)
    { // C move cursor right, B move cursor down, by 999 times to
...
{ // calculate value of E.rx; converts
...
int editorRowRxToCx(erow *row,
                    int rx)
{ // use string of erow to fill contents of render,
  // copy each char from char to render;
  int cur_rx = 0;
  int cx;
  for (cx = 0; cx < row->size;
       cx++)
  { // count tabs to know much memory to allocate for render;
    if (row->chars[cx] == '\t')
  ...
  if (fd != -1)
  {
    if (ftruncate(fd, len - 1) != -1)
    {
      if (write(fd, buf, len - 1) == len - 1)
      {
        ...
        free(buf);
        E.dirty = 0;
        editorSetStatusMessage("%d bytes written to disk", len - 1);
        return;
      }
    }
    close(fd);
...
/*** append buffer ***/
struct abuf
{ // make our own dynamic string type
  char *b;
  int len;
};
...
void editorDrawRows(
    struct abuf *ab)
{ // Draws each row, draws tilde on each empty row.
  int y;
  int current_position = 0;
  for (y = 0; y < E.screenrows; y++)
  { // display correct # of tildes;
...
void editorDrawStatusBar(
    struct abuf *ab)
{                             // make inverted background at the last line; \x1b[7m
  ...
  while (len < E.screencols)
  {
    ...
    { // show current line number on the right;
...
void editorRefreshScreen()
{ //\x1b means escape and is always followed by [;
...
int main(int argc, char *argv[])
{
  ...
  editorSetStatusMessage(
      "HELP: Save(Ctrl-S) | Quit(Ctrl-Q) | Find(Ctrl-F) | Format(Ctrl-P)");
  while (1)
  {
    editorRefreshScreen();
