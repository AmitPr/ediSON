...
enum editorKey {  // user arrow keys to move cursor
    ...
    PAGE_UP,
    PAGE_DOWN
>>>>>>> eb623bb9d92eb38fc2762680046a5587bb583b2d
};
...
void enableRawMode() {  // rawmode (texteditor mode, unlike canonical mode);
    ...
    raw.c_lflag &= ~(ECHO | ICANON | IEXTEN |
                     ISIG);  // modify struct; turn off canonical
                ...
                if (iscntrl(c[j])) {
                    ...
                    if (color != current_color) {
                        ...
                        if (editorSyntaxToColor(hl[j], 1)) {
                            clen =
                                snprintf(buf, sizeof(buf), "\x1b[%d;1m", color);
                        } else {
                            clen =
...
void editorDrawStatusBar(
    ...
    while (len < E.screencols) {
        if (E.screencols - len ==
            rlen) {  // show current line number on the right;
...
void initEditor() {
    ...
    E.screenrows = E.raw_screenrows -
                   2;  // make two lines of space at the end of the screen;
